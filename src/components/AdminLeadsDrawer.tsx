...
import { Database, Mail, Phone, Trash2, X, CheckSquare, MessageSquare, ExternalLink, Calendar, RefreshCcw } from 'lucide-react';
import { getAllEnquiries, deleteEnquiry, updateEnquiryStatus, SavedInquiry } from '../services/db';

export default function AdminLeadsDrawer() {
  const [isOpen, setIsOpen] = useState(false);
  const [leads, setLeads] = useState<SavedInquiry[]>([]);
  ...
  return (
    <>
      {/* Tiny Trigger Button on the Bottom-Right for Testing / Verification */}
          ...
          <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex justify-end">
            ...
            >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="font-space text-[10px] text-gray-300 uppercase tracking-widest">
                      Local DB persistence • Resend logs
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#F8FAFC]">
                ...
                {/* Simulated Email Dispatches Log Box */}
                  ...
                  {latestEmailLogs ? (
                    <div className="space-y-4">
                      ...
                      {latestEmailLogs.studentEmail ? (
                        ...
                        <p className="text-[10px] text-gray-400 italic">
                          No student confirmation email dispatched (email was not provided in input form).
                        </p>
                      )}
                    </div>
                  ) : (
                    <div className="p-4 bg-[#F8FAFC] rounded-lg text-center border border-dashed border-gray-200">
                      <p className="font-sans text-xs text-gray-400">
                        No inquiries submitted in this browser session yet. Submit an Inquiry form or registration form to see the Resend trigger in action!
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <h4 className="font-serif text-base font-bold text-[#0D2C54]">
                      Stored Database Entries ({leads.length})
                    </h4>
                    <button
                      onClick={loadLeads}
                      className="text-[10px] font-space uppercase text-[#D31218] font-bold hover:underline flex items-center gap-1"
                    >
                  ...
                  {leads.length > 0 ? (
                        ...
                        <div key={lead.id} className="p-4 bg-white border border-gray-100 rounded-xl shadow-xs space-y-3">
                          <div className="flex flex-wrap items-start justify-between gap-2 border-b border-gray-50 pb-2">
                            <div>
                              ...
                              <p className="font-space text-[9px] text-gray-400 uppercase">{lead.id} • {new Date(lead.createdAt).toLocaleDateString()}</p>
                            </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                            <div className="flex items-center gap-1.5 text-gray-600">
                              ...
                              <span className="font-mono font-semibold">+91 {lead.phone}</span>
                            </div>
                            <div className="flex items-center gap-1.5 text-gray-600">
                              <Mail className="w-3.5 h-3.5 text-gray-400" />
                              <span className="truncate">{lead.email || 'None Provided'}</span>
                            </div>
                          </div>

                          <div className="bg-[#F8FAFC] p-2.5 rounded text-xs text-gray-700 space-y-1">
                            <p className="font-semibold text-[#0D2C54] text-[10px] font-space uppercase">Target Course:</p>
                            <p className="font-sans font-medium">{lead.course.replace(/_/g, ' ').toUpperCase()}</p>
                            {lead.message && (
                              <>
                                <p className="font-semibold text-[#0D2C54] text-[10px] font-space uppercase mt-1.5">Student Notes:</p>
                    ...
                    <div className="p-8 text-center bg-white rounded-xl border border-gray-100">
                      <p className="font-sans text-xs text-gray-400">No leads stored. Complete an enquiry form to populate!</p>
                    </div>
                  )}
                </div>
              ...
              <div className="p-4 bg-gray-50 border-t border-gray-100 text-center">
                <p className="text-[10px] font-space text-gray-400 uppercase tracking-wider">
                  D'code IAS Academy lead database • ESTD 2026
                </p>
              </div>
            </motion.div>
          </div>
